...
export default function WorkOrderForm({ onClose, initial, prefillCustomer, prefillPhone, prefillPlate, prefillVehicle, prefillVisit }: Props) {
  ...
  // ===== Totals =====
  // âڑ ï¸ڈ totalCost المخزّن = الإجمالي الكامل قبل حسم الدفعات (الإيراد المحاسبي).
    ...
    <div className="space-y-4 py-2">
      ...
      {!isEdit && form.parentOrderNumber && (
        <div className="rounded-lg border border-info/30 bg-info/10 p-3 text-xs text-foreground">
          تم فتح هذا الأمر كزيارة جديدة للمركبة بعد التسليم.
          <span className="font-semibold"> الأمر السابق: {form.parentOrderNumber}</span>
          {form.visitNumber ? <span> أ¢â‚¬آ¢ رقم الزيارة: {form.visitNumber}</span> : null}
        </div>
      )}
      ...
      <div className="flex items-center justify-between gap-2 bg-primary/5 border border-primary/20 rounded-lg p-3" style={{ display: wizardVisible(0) ? undefined : "none" }}>
        <div className="text-xs">
          <div className="font-medium text-foreground">âڑ، تعبئة سريعة بالذكاء الاصطناعي</div>
            ...
            <div className="space-y-2">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-muted-foreground">
                    {vehicleMatch.brand || "—"} {vehicleMatch.model || ""} {vehicleMatch.year || ""} آ· العميل: {vehicleMatch.customer_name || "—"}
                  </p>
      ...
      <div className="border-2 border-primary/20 rounded-xl bg-card p-3 space-y-3" style={{ display: wizardVisible(2) ? undefined : "none" }}>
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm font-bold text-foreground">ًں“‹ بنود الأعمال المطلوبة</div>

...
import {
  ...
  CircleDollarSign, ClipboardList, Download, FileText, History, MapPin, Package, Printer, Receipt,
  Search, Shield, Signature, TrendingDown, TrendingUp, UserRound, WalletCards, Wrench,
} from "lucide-react";
...
import { withVehicleReturnContext } from "@/lib/vehicleNavigation";
...
export function VehicleOverviewPanel({ data, tx }: { data: Vehicle360Snapshot; tx: Tx }) {
  ...
  return <div className="space-y-4">
    ...
    {data.media.length > 0 && <div className="rounded-xl border bg-card p-4"><div className="mb-3 font-bold">{tx("آخر الصور", "Latest photos")}</div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-6">{data.media.filter((row) => row.media_type !== "document" && row.public_url).slice(0, 6).map((row) => <a key={row.id} href={row.public_url} target="_blank" rel="noreferrer" className="overflow-hidden rounded-lg border"><img src={row.public_url} alt={row.caption || row.file_name || "vehicle"} loading="lazy" className="aspect-video w-full object-cover" /></a>)}</div></div>}
...
function EventRow({ row, tx, vehicleId }: { row: VehicleTimelineEvent; tx: Tx; vehicleId?: string }) {
  const navigate = useNavigate();
  ...
  return <div className="relative grid gap-3 border-b border-border/60 p-4 last:border-0 md:grid-cols-[175px_1fr_auto]">
    ...
    <div className="flex items-center gap-2 md:justify-end">{typeof row.amount === "number" && row.amount !== 0 && <span className="font-mono text-sm font-bold">{money(row.amount)}</span>}{row.href && <Button size="sm" variant="outline" onClick={() => navigate(withVehicleReturnContext(row.href!, vehicleId))}>{tx("فتح", "Open")}</Button>}</div>
  </div>;
}
...
export function VehicleTimelinePanel({ data, tx }: { data: Vehicle360Snapshot; tx: Tx }) {
  ...
  return <div className="overflow-hidden rounded-xl border bg-card">
    ...
    {filtered.length ? filtered.map((row) => <EventRow key={row.id} row={row} tx={tx} vehicleId={data.vehicleId} />) : <div className="p-12 text-center text-sm text-muted-foreground">{tx("لا توجد أحداث مطابقة", "No matching events")}</div>}
  </div>;
}
...
export function VehicleClaimsPanel({ data, tx }: { data: Vehicle360Snapshot; tx: Tx }) {
  const navigate = useNavigate();
  return <div className="space-y-4"><div className="overflow-x-auto rounded-xl border bg-card"><table className="w-full min-w-[760px] text-sm"><thead className="bg-secondary/50 text-xs text-muted-foreground"><tr><th className="p-3 text-start">{tx("رقم المطالبة", "Claim number")}</th><th className="p-3 text-start">{tx("شركة التأمين", "Insurance company")}</th><th className="p-3 text-start">{tx("الحالة", "Status")}</th><th className="p-3 text-start">{tx("أمر العمل", "Work order")}</th><th className="p-3 text-start">{tx("التقدير", "Estimate")}</th><th className="p-3 text-start">{tx("المعتمد", "Approved")}</th><th className="p-3 text-start">{tx("LPO", "LPO")}</th><th className="p-3 text-start">{tx("التسليم", "Delivery")}</th><th /></tr></thead><tbody>{data.claims.map((claim) => { const order = data.workOrders.find((row) => row.id === claim.job_order_id || row.id === claim.auto_job_order_id || row.claim_id === claim.id); return <tr key={claim.id} className="border-t"><td className="p-3 font-mono font-bold text-primary">{claim.claim_number}</td><td className="p-3">{claim.insurance_company || "—"}</td><td className="p-3">{claim.status}</td><td className="p-3 font-mono">{order?.order_number || <span className="text-amber-600">{tx("غير مرتبط", "Not linked")}</span>}</td><td className="p-3">{money(claim.estimated_amount || claim.estimated_cost)}</td><td className="p-3">{money(claim.approved_amount || claim.lpo_amount)}</td><td className="p-3"><div className="font-mono">{claim.lpo_number || "—"}</div><div className="text-xs text-muted-foreground">{dateTime(claim.lpo_received_at || claim.lpo_date)}</div></td><td className="p-3">{dateTime(claim.vehicle_delivered_at || claim.delivered_at)}</td><td className="p-3"><Button size="sm" variant="outline" onClick={() => navigate(`/insurance/${claim.id}`)}>{tx("فتح", "Open")}</Button></td></tr>; })}</tbody></table>{data.claims.length === 0 && <div className="p-12 text-center text-muted-foreground">{tx("لا توجد مطالبات مرتبطة", "No linked claims")}</div>}</div>
    <FinancialTable title={tx("التقديرات المرتبطة", "Linked estimates")} headers={[tx("رقم التقدير", "Estimate number"), tx("النوع", "Type"), tx("التاريخ", "Date"), tx("قبل الضريبة", "Before VAT"), tx("الإجمالي", "Total"), tx("الحالة", "Status")]} rows={data.estimates.map((row) => ({ id: row.id, cells: [row.estimate_number || row.legacy_number, row.estimate_type || row.estimation_type, dateTime(row.estimate_date || row.created_at), money(row.subtotal || row.lump_sum_amount), money(row.total || row.lump_sum_amount), row.status], href: row.claim_id || row.converted_claim_id ? `/insurance/${row.claim_id || row.converted_claim_id}` : null }))} onOpen={(href) => href && navigate(href)} tx={tx} />
  </div>;
}
...
export function VehicleFinancialPanel({ data, tx }: { data: Vehicle360Snapshot; tx: Tx }) {
  ...
  return <div className="space-y-5">
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><SummaryCard label={tx("الإيراد قبل الضريبة", "Revenue before VAT")} value={money(f.billedBeforeVat)} tone="good" icon={CircleDollarSign} /><SummaryCard label={tx("الضريبة", "VAT") } value={money(f.vatTotal)} icon={Receipt} /><SummaryCard label={tx("المحصل فعليًا", "Actually collected")} value={money(f.totalCollected)} tone="good" icon={Banknote} /><SummaryCard label={tx("التكلفة المباشرة قبل الضريبة", "Direct cost before VAT")} value={money(f.directCostBeforeVat)} tone="warning" icon={TrendingDown} /><SummaryCard label={tx("العمالة الخارجية", "External labour")} value={money(f.externalLabor)} tone="warning" icon={Wrench} /><SummaryCard label={tx("الربح / الخسارة الحقيقي", "Actual profit / loss")} value={money(f.actualProfit)} tone={f.actualProfit >= 0 ? "good" : "bad"} icon={TrendingUp} /><SummaryCard label={tx("هامش الربح", "Profit margin")} value={`${f.profitMarginPct.toFixed(2)}%`} tone={f.profitMarginPct >= 0 ? "good" : "bad"} icon={TrendingUp} /><SummaryCard label={tx("المستحق", "Outstanding")} value={money(f.outstanding)} icon={WalletCards} /></div>
...
export function VehicleMediaPanel({ data, tx }: { data: Vehicle360Snapshot; tx: Tx }) {
  return <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5">{data.media.map((media) => <div key={media.id} className="overflow-hidden rounded-xl border bg-card">{media.public_url && media.media_type !== "document" ? <a href={media.public_url} target="_blank" rel="noreferrer"><img src={media.public_url} alt={media.caption || media.file_name || "vehicle"} loading="lazy" className="aspect-video w-full object-cover" /></a> : <div className="flex aspect-video items-center justify-center bg-secondary/40"><FileText className="h-8 w-8 text-muted-foreground" /></div>}<div className="p-3"><div className="truncate text-sm font-semibold">{media.caption || media.file_name || media.category}</div><div className="mt-1 text-xs text-muted-foreground">{dateTime(media.uploaded_at || media.created_at)} · {media.category} · {media.uploaded_by || "—"}</div>{media.public_url && <div className="mt-2 flex flex-wrap gap-1"><Button size="sm" variant="ghost" asChild><a href={media.public_url} target="_blank" rel="noreferrer">{tx("معاينة", "Preview")}</a></Button><Button size="sm" variant="ghost" asChild><a href={media.public_url} download={media.file_name || true}><Download className="me-1 h-3 w-3" />{tx("تحميل", "Download")}</a></Button><Button size="sm" variant="ghost" onClick={() => { const w = window.open(media.public_url, "_blank", "noopener,noreferrer"); if (w) setTimeout(() => w.print(), 600); }}><Printer className="me-1 h-3 w-3" />{tx("طباعة", "Print")}</Button></div>}</div></div>)}{data.media.length === 0 && <div className="col-span-full rounded-xl border p-12 text-center text-muted-foreground">{tx("لا توجد صور أو مستندات مرتبطة", "No linked photos or documents")}</div>}</div>;
}

export function VehicleSignaturesPanel({ data, tx }: { data: Vehicle360Snapshot; tx: Tx }) {
  return <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{data.signatures.map((row) => <div key={row.id} className="rounded-xl border bg-card p-4"><div className="flex items-center gap-2 font-bold"><Signature className="h-4 w-4 text-primary" />{row.signature_role || tx("توقيع", "Signature")}</div>{row.signature_data_url && <img src={row.signature_data_url} alt="signature" className="mt-3 h-24 w-full rounded border bg-white object-contain" />}<div className="mt-3 grid gap-1 text-xs text-muted-foreground"><span>{tx("الموقّع", "Signer")}: {row.signer_name || row.recipient_name || "—"}</span><span>{tx("التاريخ", "Date")}: {dateTime(row.signed_at || row.finalized_at || row.created_at)}</span><span>{tx("الموظف", "Employee")}: {row.signed_by || row.finalized_by || "—"}</span></div></div>)}{data.signatures.length === 0 && <div className="col-span-full rounded-xl border p-12 text-center text-muted-foreground">{tx("لا توجد توقيعات مرتبطة", "No linked signatures")}</div>}</div>;
}

export function VehicleCommunicationsPanel({ data, tx }: { data: Vehicle360Snapshot; tx: Tx }) {
  return <div className="overflow-hidden rounded-xl border bg-card">{data.communications.map((row) => <div key={row.id} className="grid gap-2 border-b p-4 last:border-0 md:grid-cols-[130px_170px_1fr_auto]"><div className="font-semibold uppercase text-primary">{row.channel || row.message_type || row.event_type || "—"}</div><div className="text-xs text-muted-foreground">{dateTime(row.sent_at || row.created_at)}</div><div><div className="font-medium">{row.subject || row.event_type || tx("مراسلة", "Message")}</div><div className="mt-1 line-clamp-3 text-sm text-muted-foreground">{row.body || row.message || "—"}</div><div className="mt-1 text-xs text-muted-foreground">{row.recipient_phone || row.recipient_email || row.recipient || ""}</div></div><div className="text-xs text-muted-foreground">{row.status || "—"}</div></div>)}{data.communications.length === 0 && <div className="p-12 text-center text-muted-foreground">{tx("لا توجد مراسلات مرتبطة", "No linked communications")}</div>}</div>;
}
...
export function VehicleAuditPanel({ data, tx }: { data: Vehicle360Snapshot; tx: Tx }) {
  ...
  return <div className="overflow-hidden rounded-xl border bg-card">{events.map((row) => <EventRow key={row.id} row={row} tx={tx} vehicleId={data.vehicleId} />)}{events.length === 0 && <div className="p-12 text-center text-muted-foreground">{tx("لا توجد أحداث تدقيق محفوظة", "No audit events recorded")}</div>}</div>;
}

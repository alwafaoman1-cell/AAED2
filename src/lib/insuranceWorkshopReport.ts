...
export function getInsuranceWorkshopReportHtml(data: WorkshopReportData): string {
  ...
  const singlePageWithSummaryCapacity = 8;
  const pageRows: WorkshopReportRow[][] = [];

  if (rows.length <= singlePageWithSummaryCapacity) {
    pageRows.push(rows);
  } else if (rows.length <= rowsPerPage * 2) {
    const firstCount = Math.ceil(rows.length / 2);
    pageRows.push(rows.slice(0, firstCount), rows.slice(firstCount));
  } else {
  ...
  const renderReportHeader = (pageNumber: number, totalPages: number): string => pageNumber === 1
    ? `<div class="header">
        <div class="company">
    ...
    : `<div class="continuation-header">
        <div class="continuation-company">
          ${s.logoUrl ? `<img class="continuation-logo" src="${s.logoUrl}" alt="logo" />` : ""}
          <div><strong>${s.companyName}</strong><span>تقرير عمليات الورشة — تابع</span></div>
        </div>
  ...
  const renderRowsTable = (pageData: WorkshopReportRow[], offset: number): string => `<table>
      <thead>
        <tr>
          <th>#</th>
  ...
  const renderSummary = (): string => `<div class="totals">
  ...
  let rowOffset = 0;
  const renderedPages = pageRows.map((pageData, pageIndex) => {
    const pageNumber = pageIndex + 1;
    const isLastPage = pageNumber === pageRows.length;
    const html = `<section class="page">
      ${renderReportHeader(pageNumber, pageRows.length)}
      ${renderRowsTable(pageData, rowOffset)}
      ${isLastPage ? renderSummary() : ""}
      <div class="footer">
        <span>${s.footerText ?? ""}</span>
        <span>${s.companyName} • ${new Date().toLocaleString("en-GB")} • ${pageNumber}/${pageRows.length}</span>
      </div>
    </section>`;
    rowOffset += pageData.length;
    return html;
  }).join("");

  return `<!DOCTYPE html>
<html lang="ar" dir="rtl" data-pdf-paginated="1">
<head>
<meta charset="UTF-8" />
...
.page { width: 297mm; height: 210mm; min-height: 210mm; margin: 8mm auto; background: white; padding: 10mm 12mm 16mm; box-shadow: 0 2px 20px rgba(0,0,0,0.08); position: relative; break-after: page; page-break-after: always }
...
.continuation-header { min-height: 20mm; display:flex; justify-content:space-between; align-items:center; border-bottom:3px solid ${s.primaryColor}; padding-bottom:7px; margin-bottom:9px }
.continuation-company { display:flex; align-items:center; gap:10px }
.continuation-company > div { display:flex; flex-direction:column; gap:2px }
.continuation-company strong { font-size:15px }
.continuation-company span, .continuation-meta span { font-size:9px; color:#6b7280 }
.continuation-logo { max-width:34px; max-height:34px }
.continuation-meta { display:flex; flex-direction:column; gap:2px; text-align:left }
.continuation-meta strong { font-size:11px }
...
table { width: 100%; border-collapse: collapse; font-size: 7.5px; line-height: 1.25; table-layout: fixed }
thead { display: table-header-group }
tr { break-inside: avoid; page-break-inside: avoid }
thead th { background: #1a1a2e; color: white; padding: 5px 3px; text-align: center; font-weight: 600; font-size: 7.5px; line-height:1.3; border: 1px solid #1a1a2e }
tbody td { padding: 3px 2px; border: 1px solid #ddd; text-align: center; vertical-align: middle; overflow-wrap:anywhere }
tbody tr:nth-child(even) { background: #fafbfc }
.num { font-family: monospace; direction: ltr; text-align: left }
.idx { background: #eef2f7; font-weight: 700; color: #555; width: 22px }
...
.footer { position:absolute; right:12mm; left:12mm; bottom:5mm; padding-top:5px; border-top:1px solid #ddd; display:flex; justify-content:space-between; font-size:8px; color:#888 }
@media print { html, body { background:white !important; margin:0 !important } .page { box-shadow:none !important; margin:0 !important; width:297mm !important; height:210mm !important } }
</style>
</head>
<body>
  ${renderedPages}

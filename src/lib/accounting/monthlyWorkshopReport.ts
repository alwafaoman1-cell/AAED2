...
export function exportMonthlyWorkshopWorkbook(input: {
  ...
  XLSX.utils.book_append_sheet(wb, sheet([
    ...
    ["مصروفات قطع الغيار", cashSummary.parts_cost || 0, insuranceSummary.parts_cost || 0, Number(cashSummary.parts_cost || 0) + Number(insuranceSummary.parts_cost || 0)],
  ...
  const vehicleHeaders = ["النوع","أمر العمل","المطالبة","العميل","الهاتف","اللوحة","الماركة","الموديل","الفواتير","الإيراد المحقق قبل الضريبة","أجرة العمل/الخدمة المحققة","مصروفات قطع الغيار","VAT المحقق","المحصل","المستحق","تكلفة عمالة خارجية","تكاليف خارجية مباشرة","التكلفة المباشرة للشهر","الربح/الخسارة للشهر"];
  const vehicleData = (kind: string, rows: MonthlyVehicleProfitabilityRow[]) => rows.map((row) => [kind,row.work_order_number,row.claim_number,row.customer_name,row.customer_phone,`${row.plate_number || ""} ${row.plate_letters || ""}`.trim(),row.brand,row.model,row.invoice_numbers,row.invoiced_ex_vat,row.labor_revenue,row.parts_cost,row.vat,row.collected,row.outstanding,row.labor_cost,row.external_direct_cost,row.direct_cost,row.gross_profit]);
  XLSX.utils.book_append_sheet(wb, sheet([vehicleHeaders,...vehicleData("كاش",cashRows),...vehicleData("تأمين",insuranceRows)], [12,18,22,28,16,16,16,18,24,16,22,20,14,16,16,20,22,18,18]), "ربحية السيارات");

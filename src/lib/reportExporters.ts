...
function buildReportHtml(p: ReportExportPayload): string {
  ...
  const english = p.language === "en";
  const direction = english ? "ltr" : "rtl";
  const generatedAt = p.generatedAt || new Date().toISOString();
  ...
  return `<!DOCTYPE html>
<html dir="${direction}" lang="${english ? "en" : "ar"}">
<head>
<meta charset="utf-8" />
  ...
  <div class="page">
    <div class="head">
      <div class="brand-wrap">
        ${template.logoUrl ? `<img class="brand-logo" src="${escapeHtml(template.logoUrl)}" alt="Logo" />` : ""}
        <div class="brand">
          <h1>${escapeHtml(english ? template.companyNameEn : template.companyName)}</h1>
          <small>${escapeHtml(english ? template.companyName : template.companyNameEn)}</small>
          <div class="meta">CR: ${escapeHtml(template.commercialReg || "—")} · VAT: ${escapeHtml(template.vatNumber || "—")}</div>
        </div>
      </div>
      <div class="doc-title">${escapeHtml(p.title)}</div>
    </div>
    ${p.subtitle ? `<div class="meta">${escapeHtml(p.subtitle)}</div>` : ""}
    ${p.rangeLabel ? `<div class="meta">${english ? "Period" : "الفترة"}: ${escapeHtml(p.rangeLabel)}</div>` : ""}
    <div class="meta">${english ? "Generated by" : "أنشئ بواسطة"}: ${escapeHtml(p.generatedBy || "Authenticated user")} · ${escapeHtml(generatedAt)}</div>
    <table>
    ...
    ${stampHtml}
    <div class="footer">
      <span>${english ? "Generated" : "صدر بتاريخ"}: ${escapeHtml(generatedAt)}</span>
      <span>${escapeHtml(template.companyNameEn)}</span>
    </div>
  </div>
</body>
</html>`;

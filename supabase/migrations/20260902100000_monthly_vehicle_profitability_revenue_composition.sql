-- Monthly cash/insurance vehicle profitability with invoice revenue composition.
...
-- Direct vehicle costs = all eligible linked expenses for operations with activity in the selected period.
...
begin
  ...
  invoice_line_amounts as (
    select
      ...
      i.source_type,
      i.subtotal,
      coalesce(sum(line.net_amount), 0)::numeric as raw_line_total,
      coalesce(sum(line.net_amount) filter (where line.is_parts), 0)::numeric as raw_parts_total
    from period_invoices i
    left join public.sales_documents sd
      on i.source_type = 'customer_invoice'
     and sd.tenant_id = i.tenant_id
     and sd.id = i.id
    left join public.insurance_invoices ii
      on i.source_type = 'insurance_invoice'
    ...
    left join lateral (
      select
        (
          ...
          * case
              when pg_input_is_valid(coalesce(item->>'unitPrice', item->>'unit_price', item->>'price'), 'numeric')
                then greatest(coalesce(item->>'unitPrice', item->>'unit_price', item->>'price')::numeric, 0)
              else 0
            end
          * (
              1 - least(greatest(
                case when pg_input_is_valid(item->>'discount', 'numeric')
                  then (item->>'discount')::numeric else 0 end,
                0
              ), 100) / 100
            )
      ...
      from jsonb_array_elements(
        case
          when i.source_type = 'customer_invoice' then coalesce(sd.items, '[]'::jsonb)
          when i.source_type = 'insurance_invoice' then coalesce(ii.items, '[]'::jsonb)
          else '[]'::jsonb
        end
      ) item
    ) line on true
    group by i.tenant_id, i.id, i.source_type, i.subtotal
  ),
  invoice_month as (
    select coalesce(i.work_order_id::text, 'vehicle:' || i.vehicle_id::text) operation_key,
      ...
      sum(i.subtotal)::numeric invoiced_ex_vat,
      sum(
        case when lm.raw_line_total > 0
          then i.subtotal * lm.raw_parts_total / lm.raw_line_total
          else 0
        end
      )::numeric parts_revenue,
      sum(
        i.subtotal - case when lm.raw_line_total > 0
          then i.subtotal * lm.raw_parts_total / lm.raw_line_total
          else 0
        end
      )::numeric labor_revenue,
      sum(i.vat)::numeric vat, sum(i.total)::numeric invoiced_total
    from period_invoices i
    left join invoice_line_amounts lm
      on lm.tenant_id = i.tenant_id
     and lm.id = i.id
     and lm.source_type = i.source_type
  ...
  report_rows as (
    select
      ...
      coalesce(im.labor_revenue,0)::numeric labor_revenue,
      coalesce(im.parts_revenue,0)::numeric parts_revenue,
    ...
    left join expense_lifetime em on em.operation_key=a.operation_key
  ...
  select
    ...
    coalesce((select jsonb_build_object(
      'invoiced_ex_vat',sum(invoiced_ex_vat),'labor_revenue',sum(labor_revenue),'parts_revenue',sum(parts_revenue),
      ...
      'labor_cost',sum(labor_cost),'operating_cost',sum(operating_cost),'external_direct_cost',sum(external_direct_cost),'direct_cost',sum(direct_cost),
  ...
  return jsonb_build_object(
    ...
    'basis','invoice subtotal is split into billed labor/service and billed parts from stored invoice items; direct vehicle costs are lifetime per operation; internal billed labor is revenue, external labor vouchers are cost; VAT is excluded from profit; monthly overhead is not allocated to vehicles',
    'generatedAt',now()
  );
end;
$$;

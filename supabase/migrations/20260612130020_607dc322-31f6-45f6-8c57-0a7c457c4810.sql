...
BEGIN
  -- ينشأ أمر العمل فقط عندما تكون المطالبة معتمدة + تاريخ وصول للورشة موجود + لا يوجد أمر مرتبط بعد
  IF NEW.auto_job_order_id IS NULL
     AND NEW.status = 'approved'
     AND NEW.workshop_arrival_date IS NOT NULL THEN

    IF TG_OP = 'INSERT' THEN
      v_should_create := true;
    ELSIF TG_OP = 'UPDATE' THEN
      -- إما لحظة الانتقال للموافقة، أو إضافة تاريخ الوصول بعد الموافقة
      IF (OLD.status IS DISTINCT FROM 'approved')
         OR (OLD.workshop_arrival_date IS NULL) THEN
        v_should_create := true;
      END IF;
    END IF;
  END IF;

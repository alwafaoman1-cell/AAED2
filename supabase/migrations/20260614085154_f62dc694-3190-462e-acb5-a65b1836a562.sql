CREATE OR REPLACE FUNCTION public.get_public_work_order(p_key text, p_password text DEFAULT NULL::text)
 RETURNS TABLE(id uuid, order_number text, status text, entry_date date, created_at timestamp with time zone, updated_at timestamp with time zone, description text, diagnosis text, insurance_claim_number text, vehicle_plate text, vehicle_brand text, vehicle_model text, vehicle_year integer, vehicle_color text, customer_name text, requires_password boolean)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT jo.* INTO v_row
  FROM public.job_orders jo
  WHERE jo.id::text = p_key OR jo.order_number = p_key
  LIMIT 1;

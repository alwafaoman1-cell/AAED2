-- Matched-cash vehicle profitability.
-- Revenue is recognized from cleared payments in the selected month, net of VAT.
-- Lifetime direct vehicle costs are matched proportionally to that recognized
-- revenue, so costs follow collection without being duplicated across months.
...
begin
  ...
  return jsonb_build_object(
    ...
    'basis','matched cash basis: recognized revenue comes from cleared payments in the selected month, net of VAT and capped at linked invoice total; lifetime direct vehicle costs are matched proportionally to recognized revenue so the real vehicle profit is shown without duplicating costs across payment months; general overhead remains period-only and separate',
    'generatedAt',now()
  );
end;
$$;
...
comment on function public.monthly_vehicle_profitability_rpc(date,date,text,text,integer,integer) is
  'Matched-cash vehicle profitability: real direct costs follow recognized collections proportionally without duplicate cost recognition.';
